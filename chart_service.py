...
def show_chart_distribute_episodes(data):
    ...
    # Loại bỏ giá trị null trong cột 'Episodes' và vẽ biểu đồ histogram
    # Sử dụng 20 bins để chia dữ liệu thành các khoảng
...
def show_chart_rating_by_type(data):
    fig, ax = plt.subplots(figsize=(10, 6))
    average_rating_by_type = data.groupby('Type')['Rating'].mean().sort_values(ascending=False)
    average_rating_by_type.plot(kind='bar', color='lightgreen', edgecolor='black', alpha=0.7)
    ax.set_title('Đánh giá trung bình theo loại anime', fontsize=14)
    ax.set_xlabel('Loại anime', fontsize=12)
    ax.set_ylabel('Đánh giá trung bình', fontsize=12)
    fig.tight_layout()
    return fig
